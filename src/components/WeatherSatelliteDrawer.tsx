...
export const WeatherSatelliteDrawer: React.FC<WeatherSatelliteDrawerProps> = ({
  ...
  const coordsOk = isValidRadarCoords(venueCoords);
  const bbox = useMemo(
    () => (coordsOk ? calculateRadarBbox(venueCoords, RADAR_VIEW_KM) : null),
    [coordsOk, venueCoords.lat, venueCoords.lng],
  );
  const basemapUrl = bbox ? buildBasemapUrl(bbox) : '';

  const frames = useMemo(() => {
    const now = Date.now();
    return [25, 20, 15, 10, 5, 0].map((minsAgo) => {
  ...
  const fallbackFrame = frames[frames.length - 1] ?? { label: 'Nyt', date: new Date() };
  const currentFrame = frames[frameIndex] ?? fallbackFrame;
  const wmsUrl =
    coordsOk && currentFrame
      ? buildImageryUrl(activeLayer, venueCoords, currentFrame.date)
      : '';
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[80] flex justify-end" data-testid="weather-radar-drawer">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <div className="p-4 md:p-5 border-b border-border-subtle flex items-center justify-between bg-surface-elevated/80 backdrop-blur-md">
              ...
              <button
                ...
                className="p-2 rounded-xl hover:bg-surface-base text-text-muted hover:text-text-primary transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center cursor-pointer shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="relative rounded-2xl overflow-hidden border border-border-subtle aspect-[4/3] bg-slate-950 shadow-inner">
                {coordsOk ? (
                  <>
                    ...
                    {wmsUrl && (
                      <img
                        ...
                        className="absolute inset-0 w-full h-full object-fill pointer-events-none mix-blend-screen opacity-90"
                        onError={(e) => {
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center pointer-events-none z-10">
                      ...
                      <div className="mt-1 px-2 py-0.5 rounded-full bg-black/80 text-[10px] font-bold text-white shadow-sm whitespace-nowrap border border-white/20 max-w-[220px] truncate">
                        {venueName}
                      </div>
                    </div>
                    <div className="absolute bottom-2.5 left-2.5 px-2 py-1 rounded-lg bg-black/70 backdrop-blur-xs text-[10px] font-medium text-gray-300 border border-white/10 z-10">
                      Säde: {RADAR_VIEW_KM} km • Keskipiste: {venueName}
                    </div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center p-6 text-center text-xs text-slate-400">
                    Kentän koordinaatteja ei ole — tutkaa ei voi kohdistaa Pyrkkään.
                  </div>
                )}
              </div>
