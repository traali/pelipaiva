...
## ADDENDUM — 2026-08-28 Competitive UI/UX Agent Graph (opencode)

**Source:** `AUDIT_2026-08-28_ox-competitive-uiux-graph.md` · **Executed by:** opencode (model `hy3-free`), Chief of Staff + Team Red "BREAK" + Team Blue "OPTIMIZE". Score: Red 110 / Blue 70. All OPEN. Cross-refs to existing IDs noted.

| ID | Sev | Title | Evidence (line-cited) | Correlates | Credit |
|---|---|---|---|---|---|
| U-01 | Sev-2 | Modal focus-trap absent on ~8 dialogs | `grep role="dialog"` → only 3/8 files; Escape only SmartImport/AskCopilot/FamilyManage | M-33 | Red |
| U-02 | Sev-2 | Sticky header stacking collision (HUD vs view-switcher) | `App.tsx:640` & `:662` both `sticky top-0 z-30` | M-40 | Red |
| U-03 | Sev-3 | `window.open` ×14 missing `rel="noopener noreferrer"` (reverse tabnabbing) | `grep 'window.open('` → 14 matches, 0 with rel | — | Red |
| U-04 | Sev-3 | Form controls lack programmatic `<label>` | `QuickDropInBar.tsx:359`, `SmartImportModal.tsx:406` | — | Red |
| U-05 | Sev-3 | Sub-44px touch targets (player pills / "Muu nimi") | `QuickDropInBar.tsx:339-354` | M-34 | Red |
| U-06 | Sev-3 | Off-grid 10px spacing (`p-2.5`/`py-2.5`) | multiple components | — | Red |
| U-07 | Sev-3 | Rage-click "Navigoi paikalle" opens multiple external tabs (no guard) | `MatchdayCard.tsx:600-621` | — | Red |
| U-08 | Sev-3 | AmbientView no persistent visible exit (Escape-only) | `AmbientView.tsx:101` | — | Blue |
| U-09 | Sev-3 | Dual-purpose MatchStats CTA affordance ambiguity | `MatchdayCard.tsx:438` | — | Blue |
| U-10 | Sev-3 | Native `window.alert` for `?perhe` join errors breaks convention | `App.tsx:156` | — | Blue |
| U-11 | Sev-4 | Emoji icons not `aria-hidden` (SR noise) | many components | M-34 | Red |
| U-12 | Sev-4 | `theme-color` meta hardcoded `#000000` in light mode | `index.html` | — | Red |
| U-13 | Sev-4 | Duplicate "show past events" controls | `App.tsx:888` vs `:921` | — | Blue |

---

*Register generated by ox-alpha, 2026-08-24T17:45 @ `f1f0b4b`. Addendum appended 2026-08-28 by opencode competitive UI/UX graph. Traceability: every Sources cell lists the original audit IDs absorbed; per-finding full proofs live in the linked source audits and the 1730 proof-or-deny pass. Fix order recommendation: P0 M-01→M-11 sequentially (each ≤ half-day except M-04/M-11), then P1 numerically.*
