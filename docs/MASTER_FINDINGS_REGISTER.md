...
M-16 sync single-flight mutex · M-18 coordinate type-invariant builder · M-20 CI must run `tsc -p tsconfig.app.json` (root config checks nothing) + branch protection · M-21 ICS recurrence/TZ-description-time/uid-hash · M-23 OCR asset self-hosting *(12.5 MB traineddata downloaded to repo root by parallel session — unwired; needs placement under `public/tesseract/` + code paths before commit)* · M-31 `?share=` producer implementation-or-spec-cut · M-33 focus traps ×8 modals · M-43 ghost-tab event migration on id remap · M-48 god-module splits · M-49 storage schema v3 typing/indexes · M-52 UI polish bundle (vocabulary, 🏀 fallback, badge diet, scroll affordances) · M-54 shell deploy twin · M-53 stays a watch item (precache 1455 KiB).
